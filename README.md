...
- **Backup plan** — if Apps Script ever feels flaky, **Cal.com** is the drop-in replacement: its free individual plan has "Requires confirmation," which reproduces this exact request → pay → approve flow (Aiden gets an email with Accept/Reject, and pending bookings can hold the slot), plus custom booking questions for the waiver and a dark-theme embed that suits the site. Calendly is *not* a fit — it has no host-approval step, so parents would get "you're confirmed!" before paying, and payments there need the $10/mo tier. Neither service can take Zelle or Cash App, so payment stays manual either way.
